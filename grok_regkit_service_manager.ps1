# grok-regkit 本地服务管理器 (public companion)
# based on local v3; paths are relative/env-driven; secrets stay in runtime/runtime_secrets.json (not committed)
# 更新记录（2026-07-17）：
...
$RegkitRoot = if ($env:GROK_REGKIT_ROOT) { $env:GROK_REGKIT_ROOT } else { Join-Path (Split-Path -Parent $Root) 'grok-regkit' }
$Grok2ApiRoot = if ($env:GROK2API_ROOT) { $env:GROK2API_ROOT } else { Join-Path $Root 'grok2api' }
$CliProxyRoot = if ($env:CLIPROXY_ROOT) { $env:CLIPROXY_ROOT } else { Join-Path $Root 'cliproxyapi' }
$GatewayRoot = if ($env:CPA_GATEWAY_ROOT) { $env:CPA_GATEWAY_ROOT } else { Join-Path $Root 'cpa_gateway' }
$WslExe = Join-Path $env:SystemRoot 'System32\wsl.exe'
$WslDistro = 'Ubuntu'
$Sub2ApiDeploy = if ($env:SUB2API_DEPLOY) { $env:SUB2API_DEPLOY } else { '/home/ubuntu/sub2api-deploy' }
...
function Start-All {
    ...
    if ((Get-ValidatedPortOwner -Port 8317 -ExpectedPattern 'cli-proxy-api\.exe') -eq 0) {
        ...
        Start-TrackedProcess -Name 'cliproxyapi' -FilePath $exe -ArgumentList @('-config',(Join-Path $CliProxyRoot 'config.yaml')) -WorkingDirectory $CliProxyRoot | Out-Null
    }

    if ((Get-ValidatedPortOwner -Port 8318 -ExpectedPattern 'cpa_gateway1\.py') -eq 0) {
        $python = 'C:\Python312\python.exe'
        $gateway = Join-Path $GatewayRoot 'cpa_gateway.py'
        $gatewayEnv = @{
...
function Get-StatusObject {
    ...
    return [ordered]@{
        ...
        services = [ordered]@{
            ...
            sub2api = [ordered]@{ port = 8080; pid = (Get-PortOwner 8080); healthy = (Test-Http 'http://127.0.0.1:8080/health'); wsl_keepalive_pid = (Get-TrackedPid -Name 'sub2api-wsl' -ExpectedPattern 'sub2api-keepalive') }
        }
    }
}
